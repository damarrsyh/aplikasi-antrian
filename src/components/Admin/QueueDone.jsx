...
const QueueDone = () => {
  ...
  useEffect(() => {
    ...
    dispatch(getType()); // Pastikan data jenis antrian diambil
  }, [dispatch]);

  // Buat mapping untuk mempercepat pencarian kode identifikasi
          ...
          queueDone?.data?.map((item) =>
            item.list_selesai?.map((queue) => (
              <div key={queue._id} className="card p-3 shadow-sm">
                ...
                <span className="fw-semibold bg-success p-2 rounded text-light">Called</span>
              </div>
              ))
            )
          ) : (
